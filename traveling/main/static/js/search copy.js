...
function showMapPoint(type) {
    ...
    console.log('Showing map point for trip:', tripId, 'type:', type);
    ...
    mapModal.onclick = function(event) {
        if (event.target === mapModal) {
            ...
            // Показываем обратно модальное окно с деталями поездки
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                ymaps.ready(function() {
                    // Создаем карту только если она еще не существует
                    if (!window.map) {
                ...
                console.error('Failed to get route point data:', data);
